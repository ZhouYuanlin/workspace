 ...
 */
@Controller
@RequestMapping("{codezzmm:codezzmm;*.?}")
public class CodeZzmmController extends BaseController{

	private final String LIST_ACTION = "redirect:/codezzmm";
	@Resource
	CodeZzmmService zzmmService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeZzmmb zzmm,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/zzmm/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = zzmmService.getCount(zzmm);
			List<CodeZzmmb> list = zzmmService.queryList(zzmm, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zzmm", zzmm);
		...
		mav.addObject("url","/codezzmm");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeZzmmb zzmm,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zzmm/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeZzmmb zzmm,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zzmm", zzmm);
				return new ModelAndView("redirect:/codezzmm/create");
			}
			zzmmService.save(zzmm);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zzmm/update");
		try{
			CodeZzmmb zzmm = zzmmService.getById(id);
			mav.addObject("zzmm",zzmm);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeZzmmb zzmm,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/zzmm/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zzmm/upload");
		return mav;
	}
	...
	public void exportExcel(CodeZzmmb p,HttpServletResponse response){
		try{
			...
			List<CodeZzmmb> list = zzmmService.queryList(p,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeZzmmb r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codezzmm.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=zzmm.xls"); 			
