 ...
 */
@Controller
@RequestMapping("{codezbb:codezbb;*.?}")
public class CodeZbbController extends BaseController{

	private final String LIST_ACTION = "redirect:/codezbb";
	@Resource
	CodeZbbService zbbService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeZbb zbb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/zbb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = zbbService.getCount(zbb);
			List<CodeZbb> list = zbbService.queryList(zbb, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zjb", zbb);
	...
	public ModelAndView create( CodeZbb zbb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zbb/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeZbb zbb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zjb", zbb);
				return new ModelAndView("redirect:/codezbb/create");
			}
			zbbService.save(zbb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zbb/update");
		try{
			CodeZbb zbb = zbbService.getById(id);
			mav.addObject("zjb",zbb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeZbb zbb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/zbb/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zbb/upload");
		return mav;
	}
	...
	public void exportExcel(CodeZbb p,HttpServletResponse response){
		try{
			...
			List<CodeZbb> list = zbbService.queryList(p,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeZbb r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codezbb.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=zbb.xls"); 			
