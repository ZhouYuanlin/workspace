 ...
 */
@Controller
@RequestMapping("{codesyd:codesyd;*.?}")
public class CodeSydController extends BaseController{

	private final String LIST_ACTION = "redirect:/codesyd";
	@Resource
	CodeSydService sydService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeSydb sydb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/sydb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = sydService.getCount(sydb);
			List<CodeSydb> list = sydService.queryList(sydb, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("sydb", sydb);
		...
		mav.addObject("url","/codesyd");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeSydb sydb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/sydb/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeSydb sydb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("sydb", sydb);
				return new ModelAndView("redirect:/codesyd/create");
			}
			sydService.save(sydb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/sydb/update");
		try{
			CodeSydb sydb = sydService.getById(id);
			mav.addObject("sydb",sydb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeSydb sydb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/sydb/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		return new ModelAndView("code/sydb/upload");
	}
	...
	public void exportExcel(CodeSydb m,HttpServletResponse response){
		try{
			...
			List<CodeSydb> list = sydService.queryByVo(m);
			int s = 1;
			for(CodeSydb r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codesydb.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=codesydb.xls"); 			
