 ...
 */
@Controller
@RequestMapping("{codexzlb:codexzlb;*.?}")
public class CodeXzlbController extends BaseController{

	private final String LIST_ACTION = "redirect:/codexzlb";
	@Resource
	CodeXzlbService lxbService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeXzlb lxb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/xzlb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = lxbService.getCount(lxb);
			List<CodeXzlb> list = lxbService.queryList(lxb, start, size);
	...
	public ModelAndView create( CodeXzlb lxb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/xzlb/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeXzlb lxb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				return new ModelAndView("redirect:/codexzlb/create");
			}
			lxbService.save(lxb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/xzlb/update");
		try{
			CodeXzlb lxb = lxbService.getById(id);
			mav.addObject("lxb",lxb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeXzlb lxb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/xzlb/update");
