 ...
 */
@Controller
@RequestMapping("{assetType:assetType;*.?}")
public class CodeAssetTypeController extends BaseController{
	private final String LIST_ACTION = "redirect:/assetType";
	@Resource
	CodeAssetTypeService codeAssetTypeService;
	@RequestMapping
	public ModelAndView index(Integer page,CodeAssetType cat,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/assetType/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = codeAssetTypeService.getCount(cat);
			List<CodeAssetType> list = codeAssetTypeService.queryList(cat, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zjb", cat);
	...
	public ModelAndView create( CodeAssetType cat,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/assetType/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeAssetType cat,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zjb", cat);
				return new ModelAndView("redirect:/assetType/create");
			}
			codeAssetTypeService.save(cat);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/assetType/update");
		try{
			CodeAssetType cat = codeAssetTypeService.getById(id);
			mav.addObject("zjb",cat);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeAssetType cat,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/assetType/update");
			...
			codeAssetTypeService.update(cat);
	...
	public ModelAndView delete(HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr,Long... id)throws Exception{
		try{
			...
			codeAssetTypeService.delete(id);
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/assetType/upload");
		return mav;
	}
	...
	public ModelAndView uploadxsl(@RequestParam("uploadFile") MultipartFile uploadFile,HttpServletRequest request, HttpServletResponse response,RedirectAttributes red)throws Exception{
		try{
			...
			for(int i=1;i<sheet.getLastRowNum()+1;i++){
				...
				CodeAssetType p = new CodeAssetType();
				if(row != null){
	...
	public void exportExcel(CodeAssetType cat,HttpServletResponse response){
		try{
			...
			List<CodeAssetType> list = codeAssetTypeService.queryList(cat,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeAssetType r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codeAssetType.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=codeAssetType.xls"); 			
