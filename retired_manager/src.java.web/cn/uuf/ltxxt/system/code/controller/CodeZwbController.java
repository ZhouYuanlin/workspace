 ...
 */
@Controller
@RequestMapping("{codezwb:codezwb;*.?}")
public class CodeZwbController extends BaseController{
	private final String LIST_ACTION = "redirect:/codezwb";
	@Resource
	CodeZwbService zwbService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeZwb zwb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/zwb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = zwbService.getCount(zwb);
			List<CodeZwb> list = zwbService.queryList(zwb, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zwb", zwb);
		...
		mav.addObject("url","/codezwb");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeZwb zwb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zwb/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeZwb zwb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zwb", zwb);
				return new ModelAndView("redirect:/codezwb/create");
			}
			zwbService.save(zwb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zwb/update");
		try{
			CodeZwb zwb = zwbService.getById(id);
			mav.addObject("zwb",zwb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeZwb zwb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/zwb/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zwb/upload");
		return mav;
	}
	...
	public void exportExcel(CodeZwb p,HttpServletResponse response){
		try{
			...
			List<CodeZwb> list = zwbService.queryList(p,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeZwb r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codezwb.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=zwb.xls"); 			
