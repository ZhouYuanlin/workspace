 ...
 */
@Controller
@RequestMapping("{codektlb:codektlb;*.?}")
public class CodeKtlbController extends BaseController{

	private final String LIST_ACTION = "redirect:/codektlb";
	@Resource
	CodeKtlbService lxbService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeKtlb lxb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/ktlb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = lxbService.getCount(lxb);
			List<CodeKtlb> list = lxbService.queryList(lxb, start, size);
	...
	public ModelAndView create( CodeKtlb lxb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktlb/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeKtlb lxb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				return new ModelAndView("redirect:/codektlb/create");
			}
			lxbService.save(lxb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktlb/update");
		try{
			CodeKtlb lxb = lxbService.getById(id);
			mav.addObject("lxb",lxb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeKtlb lxb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/ktlb/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktlb/upload");
		return mav;
	}
	...
	public void exportExcel(CodeKtlb p,HttpServletResponse response){
		try{
			...
			List<CodeKtlb> list = ktlbService.queryList(p,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeKtlb r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codektlb.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=ktlb.xls"); 			
