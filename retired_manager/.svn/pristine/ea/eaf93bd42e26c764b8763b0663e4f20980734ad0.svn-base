...
<html>
	...
	<body>
			  ...
			  <div class="row">
                    ...
                    <table class="table table-striped table-bordered table-condensed table-advance table-hover">
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td>${item.name}</td>
                                    <td>${item.sfejdw}</td>
                                    <td width="150"><a href="/codedwb/${item.id}/edit" class="btn default btn-xs black" id="showtoast">编辑</a> <a href="javascript:if(confirm('您确定要删除吗？'))location.href='/codedwb/delete?id=${item.id}'" class="btn default btn-xs black">删除</a></td>
                                </tr>
                                </c:forEach>
							...
							<div class="pagination pull-right">
								<s:pagination page="${paginate.page}" namespace="/" controller="codedwb" includeParams="true" styleClass="pages fl_r"/>
							</div>
              		</div>
				</div>
			</div>
		...
		<script>
			function doCreate(){
				$('#frm').attr("action","/codedwb/create").submit();
			}
			//修改
			function doEdit(){
				if(fnupdate()){
					...
					frm.action = "${request.contextPath}/codedwb/"+val+"/edit";
					frm.submit();
				}
			}
			//删除
			function doDel(){
				if(fnremove()){
					var frm = document.getElementById('frm');
					frm.action = "${request.contextPath}/codedwb/delete";
					frm.submit();
				}
			}
			//查询
			...
			//导出
			function doXiazai(){
				$('#frm').attr("method","post").attr('action','/codedwb/export').submit();
			}
			//导入
			function doExcel(){
				$('#frm').attr('action','/codedwb/importexecl').submit();
			}
			//下载
			function doDown(){
				$('#frm').attr('method','post').attr('action','/codedwb/download').submit();
			}
		</script>
	</body>
</html>