 ...
 */
@Controller
@RequestMapping("{codektzw:codektzw;*.?}")
public class CodeKtzwController extends BaseController{

	private final String LIST_ACTION = "redirect:/codektzw";
	@Resource
	CodeKtzwService lxbService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeKtzw lxb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/ktzw/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = lxbService.getCount(lxb);
			List<CodeKtzw> list = lxbService.queryList(lxb, start, size);
	...
	public ModelAndView create( CodeKtzw lxb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktzw/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeKtzw lxb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				return new ModelAndView("redirect:/codektzw/create");
			}
			lxbService.save(lxb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktzw/update");
		try{
			CodeKtzw lxb = lxbService.getById(id);
			mav.addObject("lxb",lxb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeKtzw lxb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/ktzw/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/ktzw/upload");
		return mav;
	}
	...
	public void exportExcel(CodeKtzw p,HttpServletResponse response){
		try{
			...
			List<CodeKtzw> list = ktzwService.queryList(p,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeKtzw r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codektzw.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=ktzw.xls"); 			
