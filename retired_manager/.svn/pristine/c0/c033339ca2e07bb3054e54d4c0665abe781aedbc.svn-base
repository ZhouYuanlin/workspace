 ...
 */
@Controller
@RequestMapping("{carApplyShowTwo:carApplyShowTwo;*.?}")
public class CarApplyShowTwoAction extends BaseController{
	
	private final String LIST_ACTION = "redirect:/carApplyShowTwo";
	...
	public ModelAndView index(Integer page,Carapply cp,Carinfo ci,String carNumber,String carDriver,String times,String onclickState){
		ModelAndView mav = new ModelAndView("car/carshow/indextwo");
		try{
			//获取车辆申请信息分页list
	...
	public ModelAndView ajaxDetail(HttpServletResponse response,Long id,String useTime){
		ModelAndView mav = new ModelAndView("car/carshow/_worktwo");
		//查询车辆信息下拉列表数据
		List<Carinfo> list = carInfoService.find();
		mav.addObject("carslist",list);
		if(!useTime.equals("12315")){
			 mav = new ModelAndView("car/carshow/_workstwo");
