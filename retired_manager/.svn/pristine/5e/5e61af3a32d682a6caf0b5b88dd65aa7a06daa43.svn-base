 ...
 */
@Controller
@RequestMapping("{codezdytjb:codezdytjb;*.?}")
public class CodeZdytjbController extends BaseController{

	private final String LIST_ACTION = "redirect:/codezdytjb";
	@Resource
	CodeZdytjbService zdytjbService;
	@Resource
	CodeGjtjbService gjcxService;
	
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeZdytjb zdytjb,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/zdytjb/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = zdytjbService.getCount(zdytjb);
			List<CodeZdytjb> list = zdytjbService.queryList(zdytjb, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zdytjb", zdytjb);
		...
		mav.addObject("url","/codezdytjb");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeZdytjb zdytjb,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zdytjb/create");
		List<CodeTjb> tjblist = gjcxService.getAll();
		mav.addObject("tjbList", tjblist);
		return mav;
	}
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeZdytjb zdytjb,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zdytjb", zdytjb);
				return new ModelAndView("redirect:/codezdytjb/create");
			}
			if(zdytjb.getTjb()!=null){
				CodeTjb tjb = gjcxService.getById(zdytjb.getTjb().getId());
				zdytjb.setTjb(tjb);
			}
			zdytjbService.save(zdytjb);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zdytjb/update");
		try{
			CodeZdytjb zdytjb = zdytjbService.getById(id);
			mav.addObject("tjb",zdytjb.getTjb());
			mav.addObject("zdytjb",zdytjb);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeZdytjb zdytjb,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/zdytjb/upload");
		return mav;
	}
	...
	public ModelAndView uploadxsl(@RequestParam("uploadFile") MultipartFile uploadFile,HttpServletRequest request, HttpServletResponse response,RedirectAttributes red)throws Exception{
		try{
			...
			for(int i=1;i<sheet.getLastRowNum()+1;i++){
				...
				CodeZdytjb zdytjb = new CodeZdytjb();
				if(row != null){
					row.getCell(0).setCellType(Cell.CELL_TYPE_STRING);
					if(row.getCell(0).getStringCellValue() != null){
						zdytjb.setCode(row.getCell(0).getStringCellValue());
							if( row.getCell(1) != null){
								row.getCell(1).setCellType(Cell.CELL_TYPE_STRING);
								zdytjb.setZdywm(row.getCell(1).getStringCellValue());
							}
							if( row.getCell(2) != null){
								row.getCell(2).setCellType(Cell.CELL_TYPE_STRING);
								zdytjb.setZdzwm(row.getCell(2).getStringCellValue());
							}
							if( row.getCell(3) != null){
								row.getCell(3).setCellType(Cell.CELL_TYPE_STRING);
								zdytjb.setBz(row.getCell(2).getStringCellValue());
							}
							if( row.getCell(4) != null){
								row.getCell(4).setCellType(Cell.CELL_TYPE_STRING);
								tjb.setId(Long.parseLong(row.getCell(4).getStringCellValue()));
								zdytjb.setTjb(tjb);
							}
							zdytjbService.save(zdytjb);
					}
				...
				for(int i = 0;i<cellTitle.length;i++){
					...
					switch (i) {
						case 0:
							...
							break;
						case 1:
							da.setCellValue(r.getZdywm());
							break;
						case 2:
							da.setCellValue(r.getZdzwm());
							break;
						case 3:
						...
						case 4:
							da.setCellValue(r.getBz());
							da.setCellValue(r.getTjb().getTjbzwm());
							break;
					}
				}
				s++;
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codezdytjb.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=codezdytjb.xls"); 			
