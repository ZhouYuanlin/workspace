 ...
 */
@Controller
@RequestMapping("{codeAsset:codeAsset;*.?}")
public class CodeAssetController extends BaseController{
	private final String LIST_ACTION = "redirect:/codeAsset";
	@Resource
	CodeAssetService codeAssetService;
	@Resource
	CodeAssetTypeService codeAssetTypeService;
	@RequestMapping
	public ModelAndView index(Integer page,CodeAsset ca,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/asset/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = codeAssetService.getCount(ca);
			List<CodeAsset> list = codeAssetService.queryList(ca, start, size);
			paginate = new Paginate(list, count, size, page);
			mav.addObject("zjb", ca);
			mav.addObject("list",list);
			mav.addObject("paginate",paginate);
			mav.addObject("zbblist",codeAssetTypeService.getAll());
		}catch(Exception e){
			mav.addObject(Constants.MESSAGE_NAME, "未找到数据");
			e.printStackTrace();
		}
		mav.addObject("url","/codeAsset");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeAsset ca,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/asset/create");
		mav.addObject("zbblist",codeAssetTypeService.getAll());
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeAsset ca,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("zjb", ca);
				return new ModelAndView("redirect:/codeAsset/create");
			}
			codeAssetService.save(ca);
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/asset/update");
		try{
			CodeAsset ca = codeAssetService.getById(id);
			mav.addObject("zbblist",codeAssetTypeService.getAll());
			mav.addObject("zjb",ca);
		}catch(Exception e){
			e.printStackTrace();
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeAsset ca,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/asset/update");
	...
	public ModelAndView importexecl(HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/asset/upload");
		return mav;
	}
	...
	public ModelAndView uploadxsl(@RequestParam("uploadFile") MultipartFile uploadFile,HttpServletRequest request, HttpServletResponse response,RedirectAttributes red)throws Exception{
		try{
			Map<String,CodeAssetType> ms = new LinkedHashMap<String,CodeAssetType>();
			List<CodeAssetType> list = codeAssetTypeService.getAll();
			for(CodeAssetType z : list){
						...
						p.setCode(row.getCell(0).getStringCellValue());
							...
							if( row.getCell(3) != null){
								...
								if(ms.get(row.getCell(3).getStringCellValue()) != null)
									p.setAssetId(ms.get(row.getCell(3).getStringCellValue()).getId());
							}
							codeAssetService.save(p);
					}
				}
			}
	...
	public void exportExcel(CodeAsset ca,HttpServletResponse response){
		try{
			...
			List<CodeAsset> list = codeAssetService.queryList(ca,0,Integer.MAX_VALUE);
			int s = 1;
			for(CodeAsset r : list){
	...
	public void downLoad(HttpServletRequest request, HttpServletResponse response){
		try {
			String file = request.getSession().getServletContext().getRealPath("/") + "model/codeAsset.xls";
			response.reset(); 
			response.setContentType("application/vnd.ms-excel"); 
			response.setHeader("Content-Disposition","attachment;filename=asset.xls"); 			
