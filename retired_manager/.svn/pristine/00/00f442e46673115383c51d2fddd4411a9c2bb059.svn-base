...
<html>
	...
	<form id="frm" action="<c:url value='${request.contextPath}/meetingApplyShowtwo'/>" class="form-horizontal" method="post">
			  ...
			  <div class="row">
                    ...
                    <table class="table table-striped table-bordered table-condensed table-advance table-hover">
                                ...
                                <tr>
                                    ...
                                    <th width="150" Style="text-align: center;"><b>会议室名称</b></th>
                                     <c:forEach items="${ls}" var="temp" varStatus="c" begin="0" end="0">
                                ...
                                <c:forEach items="${ls}" var="temp" varStatus="c">
                                	 <tr class="odd gradeX">
                                	 	  ...
                                	 	  <c:forEach items="${temp}" var="map" begin="0" end="0">
									          <c:forEach items="${map.value}" var="carlist">
									          		<td Style="text-align: center;vertical-align:middle;"><span title="设备: ${carlist.meetingRoom.device}&#10面积: ${carlist.meetingRoom.area}&#10地址: ${carlist.meetingRoom.address}">${carlist.meetingRoom.name}<br></span></td>
									           </c:forEach>
									      </c:forEach>
									          ...
									          <td Style="text-align: center;width:12%">
										           <c:forEach items="${map.value}" var="carlist" varStatus="stauts">
											           <c:if test="${carlist.startTime != null}">	
										          	  		<a Style="margin-top:0px;width:100%;color:white;<c:if  test="${carlist.applyTime != null}">background:rgb(34, 242, 21) </c:if><c:if  test="${carlist.applyTime == null}">background:#35BCF7 </c:if>" title="申请人: ${carlist.applicantName}&#10使用原因: ${carlist.reason}&#10&#10申请时间: ${carlist.addTime}&#10审核时间: ${carlist.applyTime}"  class="btn default btn-xs black" data-toggle="modal">${carlist.startTime}-${carlist.endTime}</a><br>
										          	  		<a Style="width:100%;height:10px;opacity:0;" href="#showdetail" ref="${carlist.meetingRoom.id},${carlist.useTime}" class="btn default btn-xs black" data-toggle="modal"></a>
											         <%-- <c:if test="${stauts.index+1==fn:length(map.value)}">
											              <a Style="margin-top:0px;width:100%;color:white;<c:if  test="${carlist.applyTime != null}">background:rgb(34, 242, 21) </c:if><c:if  test="${carlist.applyTime == null}">background:#35BCF7 </c:if>" title="申请人: ${carlist.applicantName}&#10使用原因: ${carlist.reason}&#10&#10申请时间: ${carlist.addTime}&#10审核时间: ${carlist.applyTime}" href="#showdetail" ref="${carlist.meetingRoom.id},${carlist.useTime}"  class="btn default btn-xs black" data-toggle="modal">申请</a><br>
											          </c:if> --%>
											       </c:if>
											           <c:if test="${carlist.startTime == null}">	
										          	  		<a Style="width:100%;height:80px;opacity:0;" href="#showdetail" ref="${carlist.meetingRoom.id},${carlist.useTime}" class="btn default btn-xs black" data-toggle="modal"></a>
											          </c:if>
									           	   </c:forEach>
									          </td>
									      </c:forEach>
                                ...
                                <c:if test="${empty list}"><tr><td colspan="14">本周没有会议室使用数据</td></tr></c:if>
                                </tbody>
                                </table>
		...
		$(function(){
			...
			$('.black').click(function(){
				...
				var kk = id.split(",");
				$.post('/meetingApplyShowtwo/ajaxdetail?'+new Date().getTime(),{"id":kk[0],"useTime":kk[1]},function(d){
					$("#showdetail").html(d);
				}); 
			});
		});
		...
		function doSearch(){
			var k = true;
			if(k)
				$('#frm').attr('action','/meetingApplyShowtwo').submit();
			else 
				return false;
		}
