...
<html>
	...
	<body>
		...
		<script>
			function doCreate(){
				$('#frm').attr("action","/assetType/create").submit();
			}
			//修改
			function doEdit(){
				if(fnupdate()){
					...
					frm.action = "${request.contextPath}/assetType/"+val+"/edit";
					frm.submit();
				}
			}
			//删除
			function doDel(){
				if(fnremove()){
					var frm = document.getElementById('frm');
					frm.action = "${request.contextPath}/assetType/delete";
					frm.submit();
				}
			}
			//查询
			function doSearch(){
				var frm = document.getElementById('frm');
				frm.action = "/assetType";
				frm.submit();
				
			}
			//导出
			function doXiazai(){
				$('#frm').attr("method","post").attr('action','/assetType/export').submit();
			}
			//导入
			function doExcel(){
				$('#frm').attr('action','/assetType/importexecl').submit();
			}
			//下载
			function doDown(){
				$('#frm').attr('method','post').attr('action','/assetType/download').submit();
			}
		</script>
	</body>
</html>