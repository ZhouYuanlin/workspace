...
<html>
	<head></head>
	<body>
	<form id="frm" action="<c:url value='${request.contextPath}/activityReleas'/>" class="form-horizontal" method="post">
		...
		function doEdit(){
			if(fnupdate()){
				...
				$('#frm').attr('method','post').attr("action","${request.contextPath}/activityReleas/"+val+"/edit").submit();
				frm.submit();
			}
		}
		//删除
		...
		//查询
		function doSearch(){
			$('#frm').attr('action','/activityReleas').submit();
		}
		...
		function doAssign(){
			if(fnupdate()){
				...
				$('#frm').attr('method','post').attr("action","${request.contextPath}/activityReleas/"+val+"/assign").submit();
				frm.submit();
			}
		}
		</script>
