...
<html>
	...
	<form id="frm" action="<c:url value='${request.contextPath}/meetingApplyShow'/>" class="form-horizontal" method="post">
			  ...
			  <div class="row">
                    ...
                    <table class="table table-striped table-bordered table-condensed table-advance table-hover">
                                ...
                                <c:forEach items="${ls}" var="temp" varStatus="c">
                                	 <tr class="odd gradeX">
                                	 	  <td Style="text-align: center;vertical-align:middle;" >${c.index + 1}</td>
                                	 	  <c:forEach items="${temp}" var="map" begin="0" end="0">
									          <c:forEach items="${map.value}" var="carlist">
		...
		$(function(){
			...
			$('.black').click(function(){
				...
				var kk = id.split(",");
				$.post('/meetingApplyShow/ajaxdetail?'+new Date().getTime(),{"id":kk[0],"useTime":kk[1]},function(d){
					$("#showdetail").html(d);
				}); 
			});
		});
		...
		function doSearch(){
			var k = true;
			if(k)
				$('#frm').attr('action','/meetingApplyShow').submit();
			else 
				return false;
		}
