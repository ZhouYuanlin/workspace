 ...
 */
@Controller
@RequestMapping("{codemode:codemode;*.?}")
public class CodeModeController extends BaseController{

	private final String LIST_ACTION = "redirect:/codemode";
	@Resource
	CodeModeService modeService;
	
	@RequestMapping
	public ModelAndView index(Integer page,CodeMode mode,HttpServletRequest request,HttpServletResponse response){
		ModelAndView mav = new ModelAndView("code/mode/index");
		try{
			page = page == null || page < 0 ? 1 : page;
			int start = (page - 1) * size;
			Long count = modeService.getCount(mode);
			List<CodeMode> list = modeService.queryList(mode, start, size);
		...
		mav.addObject("url","/codemode");
		return mav;
	}
	@RequestMapping("create")
	public ModelAndView create( CodeMode mode,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/mode/create");
		return mav;
	}
	
	@RequestMapping(value="save",method=RequestMethod.POST)
	public ModelAndView save(@Valid CodeMode mode,BindingResult binding,HttpServletRequest request,HttpServletResponse response,RedirectAttributes redAttr)throws Exception{
		try{
			if(binding.hasErrors()){
				...
				redAttr.addFlashAttribute("mode", mode);
				return new ModelAndView("redirect:/codemode/create");
			}
	...
	public ModelAndView modify(@PathVariable Long id,HttpServletRequest request,HttpServletResponse response)throws Exception{
		ModelAndView mav = new ModelAndView("code/mode/update");
		try{
			CodeMode mode = modeService.getById(id);
			mav.addObject("mode",mode);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public ModelAndView update(HttpServletRequest request,HttpServletResponse response,@Valid CodeMode mode,BindingResult result,RedirectAttributes redAttr)throws Exception{
		try{
			if(result.hasErrors()){
				ModelAndView mav = new ModelAndView("code/mode/update");
