	...
	<form id="frm" action="<c:url value='${request.contextPath}/carApplyShow'/>" class="form-horizontal" method="post">
									          ...
									          <td Style="text-align: center;width:12%" >
										           <c:forEach items="${map.value}" var="carlist" varStatus="stauts">
											           <c:if test="${carlist.startTime != null}">	
										          	  		<span Style="margin-top:0px;width:100%;color:white;<c:if  test="${carlist.applyTime != null}">background:rgb(34, 242, 21) </c:if><c:if  test="${carlist.applyTime == null}">background:#35BCF7 </c:if>" title="用车人 : ${carlist.applyName}&#10电话 : ${carlist.tel}&#10用车原因: ${carlist.tel}&#10&#10申请时间: ${carlist.addTime}&#10审核时间: ${carlist.applyTime}"  class="btn default btn-xs black" data-toggle="modal">${carlist.startTime}-${carlist.endTime}</span><br>
		...
		$(function(){
			...
			$('.black').click(function(){
				...
				var kk = id.split(",");
				$.post('/carApplyShowTwo/ajaxdetail?'+new Date().getTime(),{"id":kk[0],"useTime":kk[1]},function(d){
					$("#showdetail").html(d);
				}); 
			});
		});
		...
		function doEdit(){
			if(fnupdate()){
				...
				frm.action = "${request.contextPath}/carApplyShowTwo/"+val+"/edit";
				frm.submit();
			}
		}
		...
		function doApply(){
			if(fnupdate()){
				var frm = document.getElementById('frm');
				frm.action = "${request.contextPath}/carApplyShowTwo/"+val+"/apply";
				frm.submit();
			}
		}
		...
		function doSearch(){
			var k = true;
			if(k)
				$('#frm').attr('action','/carApplyShowTwo').submit();
			else 
				return false;
		}
