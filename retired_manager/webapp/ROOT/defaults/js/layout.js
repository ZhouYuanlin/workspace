        ...
        */
        ...
        if (deviceAgent.match(/(iphone|ipod|ipad)/)) {
            $(document).on('focus', 'input, textarea', function () {
                $('.page-header').hide();
                $('.page-footer').hide();
                $('.page-footer-inner').hide();
                
            });
            $(document).on('blur', 'input, textarea', function () {
                $('.page-header').show();
                $('.page-footer').show();
                $('.page-footer-inner').show();
            });
        }
    }
...
// 2014-05-24 添加左侧菜单滚动	
        if ($('.page-sidebar-fixed,.page-sidebar-fixed-custom').size() === 0) {
            handleSidebarAndContentHeight();
			setSidebarHeight();
            return;
        }
...
// 2014-05-24 修改			
			if ($('body').hasClass("page-sidebar-closed")) {
				$('.slimScrollDiv,.slimScrollDiv ul').css({overflow:'visible'});
			}
// 		
            handleSidebarAndContentHeight();	
			setSidebarHeight();	
    ...
    // Handles the setSidebarHeight 2014/07/03
	var setSidebarHeight = function () {
		if ($('.page-sidebar-tabs')) {
			var sidebarHeight=$('.slimScrollDiv').height();
			$('.page-sidebar-tabs .tab-content .tab-pane').each(function () {
				if ($(this).children().hasClass('form-horizontal') == true) {
					$(this).find('.scroller-div').css({height:sidebarHeight-93});
				} else {
					$(this).find('.scroller-div').css({height:sidebarHeight-50});
				}
